...
public static class QueryHelper
{
    ...
    ///
    /// This is also the key <see cref="Ambiguity.Ordered"/> tallies by, and deliberately
    /// the same function rather than a second one that agrees: two symbols with the same
    /// reading here are two symbols no pattern can select between, so counting them
    /// separately would report a distinction the reader has no way to act on.
    /// </summary>
