...
public class ScipEmitterPackageCacheTests
{
    ...
    [Fact]
    public async Task EmitAsync_RefusesAPackageFolderThatContainsTheSolutionItself()
    {
        // A nuget.config is free to say <add key="globalPackagesFolder" value=".." />, and
        // NuGet resolves that against the file's own directory, so the "package cache"
        // becomes a directory the repository sits inside. Every first-party file outside
        // project_root is then inside a package folder, and external documents do not
        // degrade the index, so that code left the index with health clean and exit 0.
        // Silently dropping first-party code is the one direction this codebase does not
        // accept, so a folder that swallows the solution is not treated as a package
        // folder at all and its files stay loud.
        using var _ = new PackageCache(null);

        // No .git anywhere, so project_root is the solution's own directory and the tree
        // above it is outside the index: the layout in which the misconfiguration bites.
        using var tree = new ScipEmitterTests.TempDirectory();
        var solutionDirectory = Path.Combine(tree.Path, "src");
        Directory.CreateDirectory(solutionDirectory);
        ...
        var shared = Path.Combine(tree.Path, "Shared", "Thing.cs");

        var solution = ScipEmitterTests.SyntheticSolution(solutionDirectory, $$"""
            #line 1 "{{ScipEmitterTests.Escape(shared)}}"
            public class Thing { }
            #line default
            """);
    ...
    [Fact]
    public async Task EmitAsync_StillTreatsARealPackageFolderAsExternalWhenAnotherIsRefused()
    {
        // The guard drops the offending folder and not the classification. A machine whose
        // nuget.config names a folder that contains the solution AND a genuine cache still
        // has a genuine cache, and a file restored into it is still nobody's first-party
        // code: refusing the first must not turn Microsoft.NET.Test.Sdk into a false gap.
        using var _ = new PackageCache(null);
        ...
        File.WriteAllText(Path.Combine(solutionDirectory, "nuget.config"), $"""
            <configuration>
              <config><add key="globalPackagesFolder" value=".." /></config>
              <fallbackPackageFolders>
                <add key="Real" value="{cache.Path}" />
              </fallbackPackageFolders>
            </configuration>
            """);

        var fromCache = Path.Combine(
            cache.Path, "microsoft.net.test.sdk", "18.4.0", "build", "net8.0",
            "Microsoft.NET.Test.Sdk.Program.cs");
        var shared = Path.Combine(tree.Path, "Shared", "Thing.cs");

        var solution = ScipEmitterTests.SyntheticSolution(solutionDirectory, $$"""
            #line 1 "{{ScipEmitterTests.Escape(fromCache)}}"
            public class AutoGeneratedProgram { }
            #line 1 "{{ScipEmitterTests.Escape(shared)}}"
            public class Thing { }
            #line default
