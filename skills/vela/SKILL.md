...
`vela cache` lists every index held, with the solution each is of and its size, and `vela cache clear --all | --orphaned | --solution <path>` removes them. `vela index` also removes an index whose solution is no longer on disk, and, above a 2GB total, the least recently built - never one built in the last week, and never the one it just wrote. It says which and why on the run that does it. No query ever removes anything.
