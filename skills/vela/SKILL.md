...
- **The LSP tool**, where your host has one, for code you have just edited. It stays live, where vela needs a re-index, which takes minutes on a large solution.
- **vela** for:
  ...
  - callers, implementations, and what a change touches
...
Plugin and skills.sh installs copy this file, not the command. If it prints nothing, tell the user rather than quietly falling back to grep. vela needs:
...
vela uses the solution `vela.json` names, or else the only `.sln` or `.slnx` here or in the nearest directory above, up to the repository root. If it cannot pick one, pass `--solution <path>` to every command.
...
If a project fails to load or compile, vela says so, and whatever depended on it is missing.
...
vela impls   <symbol>  # what implements or overrides it
...
- **A long answer**: `refs`, `impact` and `impls` take `--files` for a count per file, and `--limit N` to print fewer. The count stays whole.
...
Every empty answer says which absence it is. Read that line.

## What it does not do

- It does not edit, refactor or rename.
- It does not cover F#, and it does not run other languages' indexers. `impls` covers C# and VB only.
